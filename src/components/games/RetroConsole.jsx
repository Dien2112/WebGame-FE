...
const RetroConsole = ({ onGameSelect }) => {
    ...
    }, [isTransitioning, tick, activeApp, isTimerDescending, timer]); 


    useEffect(() => {
        if (isTransitioning && transitionTick > 10) { 
    ...
    }, [activeApp, scenarioState, gamesData ]);
    ...
    const handleMenuSelection = (launchId, _unused) => {
        ...
        gameLogicRef.current = new ScenarioSelectLogic(
            ...
            items,
            launchId,
            (item, gameId) => startGame(item, gameId) 
        );
    ...
    const initializeMenu = (data) => {
        ...
        gameLogicRef.current = new MenuLogic(
            ...
            menuData,
            (gameId) => handleMenuSelection(gameId, menuData)
        );
        setActiveApp('MENU');
    };
