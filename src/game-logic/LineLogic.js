...
class LineLogic extends GameLogic {
  constructor(
    ...
    if (this.setTimer && this.status.remainingTime !== undefined)
      this.setTimer(this.status.remainingTime);
  ...
  onConsolePress(action, tick) {
    ...
    if (action === BUTTONS.DOWN)
      nextCursor.r = Math.min(this.gridSize - 1, cursor.r + 1);
    if (action === BUTTONS.LEFT) nextCursor.c = Math.max(0, cursor.c - 1);
    if (action === BUTTONS.RIGHT)
      nextCursor.c = Math.min(this.gridSize - 1, cursor.c + 1);
  ...
  handleEnter() {
    ...
    if (!selected) {
      ...
      if (distR + distC === 1) {
        ...
        newBoard[cursor.r][cursor.c].color =
          newBoard[selected.r][selected.c].color;
  ...
  endGame() {
    ...
    if (this.gameId) {
      import("./utils/game-service").then((mod) =>
        mod.submitScore(this.gameId, finalScore),
      );
    }
  }
      ...
      if (animState === "CLEAR") {
        if (animData.waitTick === 0) {
          ...
          const uniqueBroken = new Set(
            animData.matches.map((m) => `${m.r},${m.c}`),
          );
        ...
        const isFalling = Array(this.gridSize)
          .fill(null)
    ...
    for (let r = 0; r < this.gridSize; r++) {
      for (let c = 0; c < this.gridSize; c++) {
        ...
        if (cell.color) {
          ...
          const isCursor =
            r === this.state.cursor.r && c === this.state.cursor.c;
          const isSelected =
            this.state.selected &&
            r === this.state.selected.r &&
  ...
  findValidMove(board) {
    for (let r = 0; r < this.gridSize; r++) {
      for (let c = 0; c < this.gridSize - 1; c++) {
        if (this.checkSwap(board, r, c, r, c + 1))
          return [
            { r, c },
            { r, c: c + 1 },
          ];
    ...
    for (let r = 0; r < this.gridSize - 1; r++) {
      for (let c = 0; c < this.gridSize; c++) {
        if (this.checkSwap(board, r, c, r + 1, c))
          return [
            { r, c },
            { r: r + 1, c },
          ];
  ...
  shuffleBoard() {
    ...
    while (!valid && attempts < 100) {
      ...
      for (let r = 0; r < this.gridSize; r++) {
        for (let c = 0; c < this.gridSize; c++) {
          if (this.state.board[r][c].color)
            colors.push(this.state.board[r][c].color);
      ...
      if (
        this.findMatches(newBoard).length === 0 &&
        this.hasValidMoves(newBoard)
      ) {
        valid = true;
      }
    }
